...
import { Play, ArrowRight, Check, Video, Box, PlayCircle as PlayCircleIcon, MessageSquare, Smartphone, Activity, Timer, Film, Calendar, Wallet, Leaf, Rocket, Gem, ClipboardList, User, Mail, Send, Loader2, CheckCircle, AlertCircle } from "lucide-react";
            ...
            <AnimatePresence>
                {showToast && isError && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 20, scale: 0.9 }}
                        transition={{ type: "spring", stiffness: 300, damping: 25 }}
                        className="fixed bottom-6 right-6 lg:bottom-10 lg:right-10 z-[100] flex items-center gap-3 rounded-lg border border-red-500/30 bg-[#0d1117]/95 px-5 py-4 shadow-[0_10px_40px_rgba(239,68,68,0.3)] backdrop-blur-md"
                    >
                        <AlertCircle className="h-6 w-6 text-red-500" />
                        <p className="text-sm font-semibold tracking-wide text-white">Submission failed. Please try again.</p>
                    </motion.div>
                )}
            </AnimatePresence>
        </>
